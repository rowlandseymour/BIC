...
::: {.definition}
For a finite Markov chain, the ***period*** of a state \(i\) is given by \(d_i = \textrm{gcd}\{n > 0; p_{ii} > 0 \}\). A state is ***aperiodic*** if \(d_i = 1\). An ***aperiodic chain*** is a chain where all states are aperiodic.
:::
...
The generalisation of the transition matrix is a ***Markov kernel*** \(P \colon S \times \mathcal{S} \to [0,1]\) on a general Borel state space \((S,\mathcal{B}(S))\), which satisfies the following properties: 
...
::: {.theorem name="Detailed Balance for dominated chains" #reversibility}
