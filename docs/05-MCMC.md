...
2. To decide if he is going to move to the proposal island, the King counts out a number of shells equal to the number of size of the island. So if island five is the proposal island, he counts out five shells. He then counts out a number of stones equal to the size of the current island. 
...
<img src="05-MCMC_files/figure-html/unnamed-chunk-1-1.png" width="672" />
...
<img src="05-MCMC_files/figure-html/unnamed-chunk-1-2.png" width="672" />
:::
...
::: {.definition #Markov}
A sequence of random variables \(Y = (Y_n)_{n \in \mathbb{N}_0}\) is a ***Markov chain*** if \(\mathbb{P}(Y_{n+1} \in \cdot \mid Y_{n}, \ldots, Y_0) = \mathbb{P}(Y_{n+1} \in \cdot \mid Y_{n})\) a.s. That is, the distribution of the next state \(Y_{n+1}\) conditional on its entire past only depends on the current state \(Y_n\).
:::

::: {.definition}
A Markov chain is called ***homogeneous*** if for any $n,m \in \mathbb{N}_0$ it holds that $\mathbb{P}(Y_{n+ m} \in \cdot \mid Y_n) = \mathbb{P}(Y_{m} \in \cdot \mid Y_0)$. That is, transition probabilities only depend on the time differences.
:::
...
::: {.definition}
If for some finite set \(S = \{i_1,\ldots,i_N\}\) we have \(\mathbb{P}(Y_n \in S) = 1\) for all $n \in \mathbb{N}_0$, we call \(Y\) a ***finite Markov chain*** and \(S\) its ***state space***.
:::
...
::: {.definition}
For a finite Markov chain, the ***period*** of a state \(i\) is given by \(d_i = \textrm{gcd}\{n > 0; P^n(i,i) > 0 \}\). A state is ***aperiodic*** if \(d_i = 1\). An ***aperiodic chain*** is a chain where all states are aperiodic.
:::


::: {.definition}
A finite Markov chain is ***irreducible*** if for any \(i,j \in S\) there exists an \(n \in \mathbb{N}_0\) such that \(\mathbb{P} (Y_n = j \mid Y_0 = i) > 0\). In other words, it is possible to move from any state to any other state in a finite number of steps.
:::
...
::: {.definition}
Suppose that there exists a probability vector \(\pi\) (that is, \(\pi_i \geq 0\) and  \(\sum_{i=1}^n \pi(i) = 1\))  that is a left-eigenvector with eigenvalue \(1\) for the transition matrix \(P\), i.e.,
\[
\pi P = \pi. 
\]
Then  \(\pi\) is called the ***stationary distribution*** of the Markov chain corresponding to \(P\).
:::

The terminology arises from the fact that if the Markov chain is started in \(\pi\), the marginal distributions \(\mathbb{P}_{Y_n}\) remain unchanged (they are ***invariant*** under time translation), since then 
...
1. the Markov chain is irreducible, and
...
3. \(\pi\) is stationary for the Markov chain.
...
1. \(\forall x \in S: \quad P(x, \cdot) \text{ is a probability measure}\), 
2. \(\forall A \in \mathcal{B}(S): \quad x \mapsto P(x, A) \text{ is measurable.}\)

For two Markov kernels \(P,Q\) we let 
\[P\circ Q(x,A) = \int_{S} P(x, dy ) Q(y,A), \quad (x,A) \in S \times \mathcal{B}(S),\]
...
Then, if we define iteratively  the Markov kernels \(P^1 = P\) and
\[P^{n+1}(x,A) = P^n \circ P(x,A) = \int_{S} P^n(x, dy) P(y, A), \quad (x,A) \in S \times \mathcal{B}(S),\]
...
::: {.example}
Let \(X= (X_n)_{n\in \mathbb{N}}\) be an i.i.d.\ sequence of \(\mathbb{R}^d\)-valued random variables with distribution \(\mathbb{P}_{X_1} = \eta \) and \(X_0 \sim \mu\) be independent of \((X_n)_{n\in \mathbb{N}}\). A ***random walk*** \(Y\) associated to the sequence of innovations \(X \) and initialistion \(X_0\) is defined by \(Y_0 = X_0\) and
\[Y_n = X_0 + \sum_{i=1}^n X_i, \quad n \in \mathbb{N},\] 
which can be equivalently characterised by the recursive relation 
\[Y_{n+1} = Y_n + X_{n+1}, \quad n \in \mathbb{N}_0. \]
This immediately yields the Markov property 
\[\mathbb{P}(Y_{n+1} \in A \mid Y_0,\ldots,Y_n) = \mathbb{P}(Y_{n+1} \in A \mid Y_n) \]
and defining for \(A -x := \{z = y -x: y \in A\} \) the Markov kernel
\[P(x,A) = \mathbb{P}(X_1 \in A - x) = \eta(A -x) \]
we obtain 
\[\mathbb{P}(Y_{n+1} \in A \mid Y_n) = \mathbb{P}(Y_n + X_{n+1} \in A \mid Y_n) = \mathbb{P}(X_{n+1} \in A - Y_n \mid Y_n) = P(Y_n, A). \]
:::

::: {.example #Langevin} 
Let \((\xi_n)_{n \in \mathbb{N}} \overset{\mathrm{i.i.d}}{\sim} N(0, I)\) and  \(V \colon \mathbb{R}^d \to \mathbb{R}\) be a continuously differentiable function. Then, for \(Y_0 \sim \mu\) independent of \((\xi_n)_{n \in \mathbb{N}} \) and a *step size* \(h > 0\) we define recursively 
\begin{align*} 
Y_{n+1} &= Y_n - h \nabla V(Y_n) + \sqrt{2h} \xi_{n+1}, \quad n \in \mathbb{N}_0.
\end{align*}
This is very reminiscent of a gradient descent algorithm with an additional stochastic perturbation that is distributed according to \(N(0,2hI) \) and therefore centered and rather small for small step sizes $h$. We call this Markov chain a *Langevin Markov Chain with step size $h$ and potential $V$*. Because the innovations are i.i.d.\ the chain is clearly Markov and since \(Y_{n +1} \mid Y_n \sim N(Y_n - h \nabla V(Y_n), 2h I ) \) its Markov kernel is given by
\[P(x,dy) = \phi_{x - h\nabla V(x), 2h I }(y) \, dy, \]
where \(\phi_{\mu,\Sigma}\) denotes the pdf of a normal random vector with mean \(\mu\) and covariance matrix \(\Sigma\).
:::

Analogously to the finite state space case we then call a distribution \(\pi\) on \(S\) a  ***stationary distribution*** if 
\[\pi P = \pi \qquad (\iff \forall A \in \mathcal{B}(S): \int \pi(dx)\, P(x,A) = \pi(A)),\]
and thus iteratively 
\[\mathbb{P}(Y_n \in \cdot \mid Y_0 \sim \pi) = \pi.\]

::: {.definition} 
A  Markov chain \(Y\) with unique stationary distribution \(\pi\) is called ***ergodic*** if 
...
::: {.corollary #doeblin}
Suppose that for some probability measure \(\nu\) and \(\alpha \in (0,1)\) the following ***Doeblin minorisation criterion*** holds: 
...
:::
::: {.remark} 
It can be shown that \(\kappa = 1 - \alpha, D=1\), see Theorem 16.0.2 in [@meyntweedie].
:::
...
::: {.theorem name="Detailed Balance" #reversibility}
Let \(P\) be a Markov kernel and $\pi$ be a distribution on the state space $S$. If $\pi$ and $P$ satisfy detailed balance in the sense
\begin{align*} &\forall x,y \in S: \pi(dx)\, P(x,dy) = \pi(dy)\, P(y,dx) \\ 
...
3. Propose a new value of $\theta'$ from some distribution $q(\cdot \mid \theta)$ for $\theta = \theta^{(i-1)}$
...
$$
otherwise set $\theta^{(i)} = \theta$ (in the acceptance probability use the convention $x/0 = +\infty$ for any $x$)
5. Repeat steps 3 to 4 for $i = 2, \ldots, n$. 
...
Let us also note that the specific choice of a posterior as target sampling density is just for the sake of exposition in this chapter. The algorithm can be applied to *any* target distribution $\pi$. 
...
\[\pi(\theta \mid \boldsymbol{y}) P(\theta, d\theta^\prime) \, d\theta = \pi(\theta^\prime \mid \boldsymbol{y}) P(\theta^\prime, d\theta) \, d\theta^\prime,  \]
and stationarity will follow from that by Theorem \@ref(thm:reversibility). The case $\theta = \theta^\prime$ is trivial, so let us  consider the case $\theta \neq \theta^\prime$. We then have
\begin{align*}
...
\]
which establishes equality of the lhs and rhs, whence detailed balance for $\theta \neq \theta^\prime$. 
::: 
...
\end{align*}
Since the rhs is independent of $\theta$, the Doeblin minorisation criterion is fulfilled with $\alpha = \rho m/M$ and Corollary \@ref(cor:doeblin) yields 
\[\sup_{x \in S} \lVert P^n(x,\cdot) - \pi(\cdot \mid \boldsymbol{y}) \rVert_{\mathrm{TV}} \leq (1-\alpha)^n = \Big(1 - \frac{\rho m}{M}\Big)^n.\]
:::

:::{.example #norm}
A counter monitors the time until atoms decays. It collects the data $Y_1, \ldots, Y_N$ and for a decay rate $\lambda$ we assume $Y_1,\ldots, Y_N \mid \lambda \overset{\mathrm{iid}}{\sim} \hbox{Exp}(\lambda)$. The time until these atom decay is long, certainly more than 1 second (or minute, hour etc.), so we assume $\lambda \sim \hbox{Beta}(\alpha, \beta)$. 

The posterior distribution is given by 
$$
\pi(\lambda \mid \boldsymbol{y}) \propto \lambda^{N + \alpha - 1} (1 - \lambda)^{\beta - 1}\exp\left(-\lambda \sum y_i\right)
$$
...
1. Decide on an starting value the Markov chain and denote it by $\lambda^{(0)}$. Set $i = 1$. 
2. Propose a new value for the parameter and denote it $\lambda'$ given $\lambda = \lambda^{(i-1)}$. In this example, we are going to propose values using a random walk method where $\lambda' \sim N(\lambda^{(i-1)}, \sigma^2)$. 
3. Accept $\lambda'$ as the value $\lambda^{(i)}$ with probability $p_{\textrm{acc}}$. Otherwise reject this value and set $\lambda^{(i)} = \lambda^{(i-1)}$.  
4. Repeat steps 2 and 3 for $i = 2, \ldots, n$. 

The value $p_{\textrm{acc}}$ is given by
$$
p_{\textrm{acc}} = \min \left\{1, \, \frac{\lambda'^{N + \alpha - 1} (1 - \lambda')^{\beta - 1}\exp\left(-\lambda' \sum y_i\right)}{\lambda^{N + \alpha - 1} (1 - \lambda)^{\beta - 1}\exp\left(-\lambda \sum y_i\right)}\right\} \\
= \min \left\{1, \, \left(\frac{\lambda'}{\lambda}\right)^{N + \alpha - 1} \left(\frac{1-\lambda'}{1-\lambda}\right)^{\beta -1} \exp\left((\lambda- \lambda')\sum y_i\right)\right\} 
$$
for $\lambda^\prime \in (0,1)$ and $0$ otherwise. As  our proposal distribution is symmetric, $q(\lambda \mid \lambda') = q(\lambda' \mid \lambda)$ and this term cancels. 
...
#set true parameter to generate data
lambda.true <- 0.3
N <- 100
...
<img src="05-MCMC_files/figure-html/unnamed-chunk-2-1.png" width="672" />

```r
#Plot posterior density
hist(lambda.store, prob = TRUE, xlab = expression(lambda), main = "Posterior density")
abline(v=lambda.true, col = 2) #the value I used to simulate the data
```

<img src="05-MCMC_files/figure-html/unnamed-chunk-2-2.png" width="672" />
...
:::{.example #norm}
The time until lorry drivers react (in milliseconds) to an obstacle in the road is 
...
<img src="05-MCMC_files/figure-html/unnamed-chunk-3-1.png" width="672" />
...
Now we have a tool to sample from posterior distributions that don't have a closed form. We can instead assign an exponential prior distribution, a distribution which only has non-negative support. Letting $\mu \sim \textrm{Exp}(10)$ sets a vague prior distribution on $\mu$. It can be shown that the posterior distribution (exercise) is therefore
$$
...
$$
q(\mu' \mid \mu) = \frac{1}{2\varepsilon} \boldsymbol{1}_{[\mu-\varepsilon,\mu + \varepsilon]}(\mu^\prime).
$$

The acceptance probability is therefore
$$
p_\textrm{acc} = \min\left\{\frac{\exp\left\{-10\mu' -\sum_{i=1}^{30}\frac{(y_i - \mu')^2}{2\sigma^2}\right\} }{\exp\left\{-10\mu -\sum_{i=1}^{30}\frac{(y_i - \mu)^2}{2\sigma^2}\right\} }, 1\right\}.
$$
We can implement a sampler in \texttt{R} as follows:
...
<img src="05-MCMC_files/figure-html/unnamed-chunk-4-1.png" width="672" />
...
<img src="05-MCMC_files/figure-html/unnamed-chunk-5-1.png" width="672" />

```r
hist(mu.store[-c(1:300)], xlab = expression(mu), main = "Posterior distribution")
```

<img src="05-MCMC_files/figure-html/unnamed-chunk-5-2.png" width="672" />

Removing such a *burn-in period* from the generated to data to ensure that the chain has *mixed* sufficiently is common practice. The necessary length of the burn-in phase generally depends on how good the initialisation relative to the target distribution is, where an arbitrary choice such as some deterministic value is often referred to as a *cold start*, whereas more informed choices are called *warm starts*. See also our later discussion on MCMC diagnostics.
...
$$
\pi(x \mid \beta) = \frac{\beta}{x^{\beta + 1}}\mathbf{1}_{(1,\infty)}(x), \quad x \in \mathbb{R}. 
$$
...
$$
\pi(\beta \mid \boldsymbol{y}) \propto \frac{\beta^{N + a - 1}e^{-b\beta}}{\prod y_i^{\beta + 1}} \mathbf{1}_{(0,\infty)}(\beta).
$$
...
$$
for $\beta^\prime > 0$ and $0$ otherwise. The MCMC algorithm will be 

1. Set an initial value $\beta^{(0)}$ and set $i =1$.
2. Propose a new value $\beta' \sim N(\beta^{(i-1)}, \sigma^2)$
3. Accept $\beta'$ with probability $p_{acc}$ and set $\beta^{(i)} = \beta'$, otherwise reject and set $\beta^{(i)} = \beta^{(i-1)}$.
4. Repeat steps 2, and 3 for \(i = 2, \ldots, n\).  
...
<img src="05-MCMC_files/figure-html/unnamed-chunk-7-1.png" width="672" />
...
<img src="05-MCMC_files/figure-html/unnamed-chunk-7-2.png" width="672" />
...
3. Repeat step 2 for \(i = 2, \ldots M\).
...
::: {.example}
In Examples 3.4 and 3.11, we had a hierarchical model with  
\begin{align*}
Y_1,\ldots,Y_N \mid (\lambda, \gamma) \sim Y_1,\ldots,Y_N \mid \lambda &\overset{\mathrm{i.i.d}}{\sim} \hbox{Exp}(\lambda) & \textrm{(likelihood)} \\
...
$$
\pi(\lambda \mid \boldsymbol{y}, \,\gamma) \propto \lambda^N \mathrm{e}^{-\lambda(\gamma + \sum_{i=1}^N y_i)},
$$
which for the specific values $N = 10$ and $\sum_{i=1}^N y_i = 95$ from the example translates to 
\[\lambda^{10}e^{-\lambda(95 + \gamma)}.\]
This shows that $\lambda \mid \boldsymbol{y}, \gamma \sim \textrm{Gamma}(11, 95 + \gamma)$. The full conditional distribution for $\gamma$ is
$$
\pi(\gamma \mid \boldsymbol{y}, \,\lambda) \propto \gamma e^{-(\nu + \lambda)\gamma}.
$$
Therefore the full conditional distribution of $\gamma$ is $\gamma \mid \boldsymbol{y}, \,\lambda \sim \hbox{Lambda}(2,\lambda + \nu)$.

We can set up a sampling algorithm using Gibbs samplers to generate samples for $\lambda$ and $\gamma$. 
...
3. Draw a value for $\lambda^{(i)} \mid \boldsymbol{y}, \gamma^{(i-1)} \sim \textrm{Gamma}(N+1, \sum_{i=1}^N + \gamma^{(i-1)})$

4. Draw a value for $\gamma^{(i)} \mid \boldsymbol{y}, \,\lambda^{(i)} \sim \Gamma(2,\lambda^{(i)} + \nu)$.

6. Repeat steps 3 and 4 for $i = 2, \ldots n$. 
...
<img src="05-MCMC_files/figure-html/unnamed-chunk-8-1.png" width="672" />

```r
plot(gamma.store, type = 'l', xlab = "iteration", ylab = expression(gamma))
```

<img src="05-MCMC_files/figure-html/unnamed-chunk-8-2.png" width="672" />
...
<img src="05-MCMC_files/figure-html/unnamed-chunk-8-3.png" width="672" />
...
<img src="05-MCMC_files/figure-html/unnamed-chunk-8-4.png" width="672" />
...
<img src="05-MCMC_files/figure-html/unnamed-chunk-8-5.png" width="672" />
:::
...
:::{.example}
Suppose $Y_1, \ldots, Y_N \mid (\beta,\theta) \overset{\mathrm{iid}}{\sim} \hbox{Weibull}(\beta, \theta)$, where
$$
\pi(y \mid\beta,\theta) = \frac{\beta}{\theta}\left(\frac{y}{\theta}\right)^{\beta - 1}\exp\left(-\frac{y^\beta}{\theta}\right), \qquad y, \beta, \theta > 0. 
$$
We use independent priors on $(\beta,\theta)$ with an Exponential prior distribution with rate $\lambda$ on $\beta$ and an inverse gamma prior distribution on $\theta$ such that
$$
\pi(\theta) \propto \frac{1}{\theta^{a + 1}}\exp\left(-\frac{b}{\theta}\right).
$$
The posterior distribution is therefore
\begin{align*}
\pi(\beta, \theta \mid \boldsymbol{y}) &\propto \pi(\boldsymbol{y} \mid \beta, \theta)\pi(\beta, \theta) \\
&= \pi(\boldsymbol{y} \mid \beta, \theta)\pi(\beta)\pi(\theta) \\
&\propto \left(\frac{\beta}{\theta^\beta}\right)^N\prod y_i^{\beta - 1}\exp\left(-\frac{1}{\theta}\sum y_i^\beta\right)
\\
...
\begin{align*}
\pi(\beta \mid \theta, \boldsymbol{y}) &\propto\left(\frac{\beta}{\theta^\beta}\right)^N\prod y_i^{\beta - 1}\exp\left(-\frac{1}{\theta}\sum y_i^\beta\right)\exp(-\lambda\beta) \\
\pi(\theta \mid \beta, \boldsymbol{y}) &\propto \frac{1}{\theta^{N + a +1}}\exp\left(-\frac{1}{\theta}(b + \sum y_i^\beta)\right) \sim \mathrm{inv-Gamma}\left(N\beta+a,b+\sum_{i=1}^N y_i^\beta\right)
\end{align*}
...
\[
p_{\textrm{acc}} = \min\left\{\frac{\pi(\beta'\mid \theta^{(i-1)},  \boldsymbol{y})}{\pi(\beta^{(i-1)} \mid \theta^{(i-1)},\boldsymbol{y})} , 1\right\}
\]
Otherwise reject $\beta'$ and set $\beta^{(i)} = \beta^{(i-1)}$. 
4. Sample $\theta^{(i)} \sim \hbox{inv}-\Gamma(N + a,\, b + \sum y_j^{\beta^{(i)}})$.
5. Repeat steps 2-4 for $i=2,\ldots,n$.

Note that by symmetry of the proposal distribution $q(\beta^\prime\mid\beta) \sum U[\beta-\varepsilon,\beta+\varepsilon], the acceptance probability in the MH-step does not depend on the proposal but only on the full conditional posterior.
:::


## MCMC Diagnostics
When running an MCMC algorithm, it is always important to check that the Markov chain has converged and is mixing well. Graphically, mixing well means the chain is exploring the space of possible values of $\theta$ efficiently and not getting stuck on the same value for a long time. A key way of doing this is by looking at the trace plot, which is a time series of the posterior samples simulated by the algorithm. The trace plot should look like it has converged to the stationary distribution and exploring the stationary distribution efficiently. What it shouldn't look like is a long series of small steps, or being stuck in one spot for a long time. 


On a more technical level, we have the following definition.

::: {.definition}
Let $Y$ be a Markov chain with state space $S$ and stationary distribution $\pi$. Given a threshold parameter $\varepsilon$ and an initial distribution $\mu$, the *mixing time* of $Y$ started in $\mu$ is given by 
\[\mathrm{t}_{\mathrm{mix}}(\varepsilon,\mu) := \inf\{n\geq 0: \lVert \mathbb{P}(Y_n \in \cdot \mid Y_0 \sim \mu) - \pi \rVert_{\mathrm{TV}} \leq \varepsilon\}. \]
If 
\[\mathrm{t}_{\mathrm{mix}}(\varepsilon) := \inf\{n\geq 0: \sup_{x \in S} \lVert \mathbb{P}(Y_n \in \cdot \mid Y_0 = x) - \pi \rVert_{\mathrm{TV}} \leq \varepsilon\} \] 
is finite, this quantity is referred to as *the* mixing time of the Markov chain for the threshold parameter $\varepsilon$.
::: 

::: {.example} 
Consider an ergodic Markov chain $Y$ with stationary distribution $\pi$ that satisfies an $(\alpha,\nu)$-Doeblin minorisation condition for some $\alpha \in (0,1)$, that is,
\[\inf_{x \in S} P(x,A) \geq \alpha \nu(A), \quad A \in \mathcal{B}(S).\] 
Then, as we have seen before, 
\[\sup_{x \in S} \lVert P^n(x, \cdot) - \pi \rVert_{\mathrm{TV}} \leq (1-\alpha)^n, \]
which yields a mixing time 
\[ \mathrm{t}_{\mathrm{mix}}(\varepsilon) \leq \frac{1}{\log(1/(1-\alpha))} \log(1/\varepsilon),\]
i.e., at most a logarithmic dependence of the mixing time on $1/\varepsilon$.
:::



::: {.definition}
If we discard the first $m$ samples of a simulated Markov chain, we call $m$ **burn-in period** of the algorithm.
:::

The burn-in period of a Markov chain simulated with some MCMC algorithm should therefore ideally be informed by its mixing time. The practical effect is the following: a natural estimator of the posterior expectation $\pi(f \mid \boldsymbol{y}) := \int f(\theta) \, d\pi(\theta \mid \boldsymbol{y})$ (e.g., for $f = \mathbf{1}_A$ this corresponds to the posterior probability $\pi(A \mid \boldsymbol{y})$ or for $f(z) = z$ this gives the posterior expectation) is given by the empirical mean 
\[ \frac{1}{n - m} \sum_{i= 0}^{n-m -1} f(Y_{i+m}).\]
If the Markov chain is ergodic, then we have a Law of Large Numbers -- in this context referred to as the ***ergodic theorem*** -- which guarantees that for any burn-in $m$, almost surely 
\[\lim_{n \to \infty} \frac{1}{n - m} \sum_{i= 0}^{n-m -1} f(Y_{i+m}) = \pi(f \mid \boldsymbol{y}), \]
provided the expectation on the rhs is well-defined and finite. However, the finite sample performance for a smart choice of $m$ might be significantly better, e.g.\ when considering the concentration probability 
\[\mathbb{P}\Big(\Big \lvert \frac{1}{n - m} \sum_{i= 0}^{n-m -1} f(Y_{i+m}) - \pi(f \mid \boldsymbol{y}) \Big\rvert \geq \varepsilon\Big). \]
Getting useful bounds for these types of probabilities, i.e.\ good concentration inequalities, is strongly related to the ergodic properties of the Markov chain (how fast does the chain converge/how fast does it mix?).
...
::: {.example}
In Example \@ref(exm:norm), we saw a Markov chain that mixes well. We took the burn-in period to be 300 iterations, which was how long it took to for the chain to converge (visually). Although the posterior distribution is invariant to the choice of the proposal distribution, it still has a large effect of the efficiency of the algorithm and how well the chain mixes. The ideal trace plot looks like white noise, or a hairy caterpillar. 
...
<img src="05-MCMC_files/figure-html/unnamed-chunk-9-1.png" width="672" />
...
<img src="05-MCMC_files/figure-html/unnamed-chunk-10-1.png" width="672" />
:::
...
:::{.example}
Suppose we have a density function that is uniformly distributed over the volume of a Euclidean ball in $N$ dimensions with radius $r$. The ball has volume
$$
V = \frac{\pi^{N/2}}{\Gamma(\frac{N}{2} + 1)}r^N.
$$
Now consider a smaller ball inside of our original ball This still has dimension $N$ but has radius $r_1 < r$. This smaller ball has volume
$$
V_1 = \frac{\pi^{N/2}}{\Gamma(\frac{N}{2} + 1)}r_1^N.
$$
The volume ratio is
$$
\frac{V_1}{V} = \Big(\frac{r_1}{r}\Big)^N,
$$
which converges geometrically fast to $0$ in the dimension $N$. This means that lots of the probability mass is concentrated in an increasingly smaller neighborhood of the boundary of the larger ball, far away from its center.
:::

This example shows that in large dimensions we need our Markov chain to spend lots of time away from the posterior mode and in the tails of the distribution, but this is where the proposal distribution has lowest mass. We can avoid this by updating each parameter individually, but this means we need to use the full conditional distributions, which have highest mass near the mode. This 'curse' makes MCMC algorithms less efficient in larger dimension, which makes it fundamentally important to track the influence of the dimension (polynomial vs.\ exponential) on their speed of convergence. 

## Beyond MCMC
MCMC is not the only method available to generate samples from the posterior distribution. MCMC is often slow and inefficient. Much of the work in computational statistics research is about developing fast and efficient methods for sampling from posterior distributions. Another method, called approximate Bayesian computation, has been discussed in the previous chapter. The Langevin Markov chain explored in examples from this chapter is another popular method that performs particularly well for convex potentials, even in high dimensions. Two other methods, beyond the scope of this module, are Sequential Monte Carlo and Hamiltonian Monte Carlo. 
...
$$
\pi(x\mid \alpha, \beta) = \frac{\beta^\alpha}{\Gamma(\alpha)}(1/x)^{\alpha + 1}e^{-\beta/x }\mathbf{1}_{(0,\infty)}(x), \quad x \in \mathbb{R}.
$$
...
Develop a code for an MCMC algorithm to sample from the posterior distribution by alternating between sampling $\alpha$ and then $\beta$. 
