...
::: {.example}
We would like to sample from an exponential distribution with rate 0.5, i.e. $Y ~ \sim \hbox{Exp}(0.5)$. The density function is given by

$$
\pi(y \mid \lambda = 0.5) = \begin{cases} 
      0.5e^{0.5y} & y \geq 0 \\
    0  & \text{otherwise.}
   \end{cases}
$$
...
$$
Therefore, if $U \sim U[0, 1]$, then it follows that $-2\log(1-U) \sim \hbox{Exp}(0.5)$. 
...
::: {.definition}
A Markov chain is **irreducible** if there exists an $n \in \mathbb{N}$ such that $\mathbb{P} (Y_n = i \mid Y_0 = j)$ for all pairs $i$ and $j$. In other words, it is possible to move from any state to any other state in a finite number of steps. 
:::
...
>>>>>>> 944450e3dd449fc0f3707f3b8260ca47bdeaba6f
...
::: {.exercise} 
Let $Y$ be a random variable with a probability density function defined by
$$
\pi(y) = \alpha y^3 \quad \hbox{for } 0 \leq y \leq 4,
$$
where $\alpha \in \mathbb{R}$ is a constant.

1. Compute the value of $\alpha$. 
...
1. Using the fact that $\int_0^4 \pi(y) dy = 1$, we have $\alpha = \frac{1}{64}$.
2. Introducing a dummy variable $x$, we have
$$
\int_0^y \frac{1}{64}x^3dx = \frac{1}{256}y^4.
$$
Hence the distribution function for $Y$ is given by
$$
F(y) = \begin{cases}
0 & \hbox{for } y \leq 0\\
...
1. Using integration by substitution, derive the distribution function $F(y)$. 
...
i. Sample $U\sim U[0, 1]$
ii. Compute $Y=(\log(1-U))^2$
:::
...
i. Sample $x$ from the exponential distribution. 
ii. Sample $u \sim U[0, 1]$. 
iii. If $u <  \frac{\pi(x)}{Mq(x)}$, then accept $x$ as a sample from the half normal distribution. 
