...
\hypersetup{
  ...
  pdfauthor={Dr Mengchu Li and Dr Lukas Trottner (based on lecture notes by Dr Rowland Seymour)},
  hidelinks,
...
\]
where \(f(x; {\theta})\) is the probability density or mass function depending on whether \(P_{\theta}\) is continuous or discrete. We shall simply refer to it as the density function of \(P_{\theta}\). The actual observed data \(\boldsymbol{y} = (y_1,\dotsc,y_n)\) are considered to be realisations of the random variable \(Y = (Y_1,\dotsc,Y_n)\).
...
\begin{definition}
The maximum likelihood estimator of \(\theta\), \(\hat{\theta}\), is the value such that \(\hat{\theta} = \mathop{\mathrm{arg\,max}}_{\theta} \pi(\boldsymbol{y}\mid \theta)\).
\end{definition}

The key to quantify uncertainty in the frequentist paradim is using confidence intervals. around the maximum likelihood estimate is based on the theory of long running events that underpin frequentist theory.

\begin{definition}
An interval \([L_n(Y), U_n(Y)] \subseteq \mathbb{R}\), is called a confidence interval (CI) at level \(1 - \alpha \in (0,1)\) for \(\theta\), if
\[ \mathbb{P}_\theta\big(L_n(Y) \leq \theta \leq U_n(Y)\big) = 1 - \alpha.
\]
\end{definition}

It is important to stress that the confidence interval (CI) \(C_n(X)\) is a random quantity and the parameter \(\theta\) is fixed (non-random) in the frequentist paradigm! The correct interpretation of CIs is that if we construct many confidence intervals from repeated random samples, \(100(1-\alpha)\%\) of these intervals would contain the true parameter \(\theta\). It does \emph{not} mean that there is a \(100(1-\alpha)\) probability that a particular interval contains the true value of \(\theta\). Note that the CI interpretation is based on the previous definition of the probability of an event and this is why CIs based inference are considered to be frequentist.

Note that, in many cases, one can only hope to obtain intervals that contains \(\theta\) when \(n\) is sufficiently large. For example, asymptotic properties of MLE
\[
...
\[
I(\theta) = \mathrm{Var} \left[ \frac{\partial}{\partial \theta} \log f(Y; \theta) \right] = -\mathbb{E} \left[ \frac{\partial^2}{\partial \theta^2} \log f(Y, \theta) \right], \qquad Y \sim \pi(y\mid\theta) 
\]
...
\begin{definition}
Two events \(A\) and \(B\) are \textbf{conditionally independent} given event \(C\) if and only if
\[ \pi(A \cap B \mid C) = \pi(A \mid C)\pi(B \mid C).\]
\end{definition}

\begin{definition}
For two random variables \(X,Y\) that have a joint pdf \(\pi(x,y)\).
...
\item
  An extension to 4 is that suppose \(Z\) is another random variable, then we have
  \[
...
For a sequence of random variables \(X_1,\dotsc,X_n\), we can factorise the joint pdf in many ways, e.g.~
\[
...
\begin{definition}
Let \(\pi(y_1, \ldots, y_N)\) be the joint density of \(Y_1, \ldots, Y_N\). If \(\pi(y_1, \ldots, y_N) = \pi(y_{\sigma_1}, \ldots, y_{\sigma_N})\) for a permutations \(\sigma\) of \(\{1, \ldots, N\}\), then \(Y_1, \ldots, Y_N\) are \textbf{exchangeable}.
\end{definition}

Exchangeability means that the labels of the random variables don't contain any information about the outcomes. This is an important idea in many areas of probability and statistics, and it is a weak assumption compared to saying \(Y_1,\dotsc,Y_N\) are independent.
...
\begin{example}
Let \((X, Y)\) follow a bivariate normal distribution with mean \textbf{0}, variances \(\sigma_x^2 = \sigma_y^2 = 1\) and a correlation parameter \(\rho \in [-1, 1]\). \((X, Y)\) are exchangeable, but only independent if \(\rho = 0\).
\end{example}

\begin{proposition}
If \(\theta \sim \pi(\theta)\) and \((Y_1, \ldots, Y_N)\) are conditionally iid given some parameter \(\theta\), then marginally \(Y_1, \ldots, Y_N\) are exchangeable.
\end{proposition}
...
& = \int \left\{\prod_{i=1}^N\pi(y_{\sigma_i} \mid \theta)\right\} \pi(\theta)\, d\theta \qquad \textrm{(product is commutative)} \\
 & = \pi(y_{\pi_1}, \ldots, y_{\sigma_N}) \qquad 
\end{split}
\end{equation}
\end{proof}
...
\item
  The \textbf{likelihood function} \(\pi(y \mid \theta)\). The likelihood function is common to both frequentist and Bayesian methods.
\item
  The \textbf{prior distribution} \(\pi(\theta)\). This is the distribution that describes our prior beliefs about the value of \(\theta\). The form of \(\theta\) should be decided \textbf{before we see the data}. It may be a vague distribution (e.g.~\(\theta \sim N(0, 10^2)\)) or a specific distribution based on prior information from experts (e.g.~\(\theta \sim N(5.5, 1.3^2)\)).\\
\item
...
\def\labelenumi{\arabic{enumi}.}
\item
  We can be very vague about \(\theta\) -- we genuinely don't know about its value. We assign a uniform prior distribution to \(\theta\) that takes values between -1,000 and +1,000, i.e.~\(\theta \sim u[-1000, 1000]\). We can write explicitly its distribution as
  \[
   \pi(\theta) =  \begin{cases} 
  ...
  \]
  {]}
  Up to proportionality, we have \(\pi(\theta) \propto 1\) for \(\theta \in [-1000, 1000]\).
\item
...
\pandocbounded{\includegraphics[keepaspectratio]{main_files/figure-latex/prior-posterior-grid-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{main_files/figure-latex/unnamed-chunk-3-1.pdf}}
...
\begin{verbatim}
##  [1] -0.587508319  0.898055935  0.407552768 -0.830248269  0.759832296
##  [6] -0.930477934 -0.263275054  0.661190481  0.002115915 -0.007060681
\end{verbatim}
...
\pandocbounded{\includegraphics[keepaspectratio]{main_files/figure-latex/unnamed-chunk-9-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{main_files/figure-latex/unnamed-chunk-9-2.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{main_files/figure-latex/unnamed-chunk-11-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{main_files/figure-latex/unnamed-chunk-14-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{main_files/figure-latex/unnamed-chunk-16-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{main_files/figure-latex/unnamed-chunk-17-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{main_files/figure-latex/unnamed-chunk-17-2.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{main_files/figure-latex/unnamed-chunk-18-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{main_files/figure-latex/unnamed-chunk-18-2.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{main_files/figure-latex/unnamed-chunk-19-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{main_files/figure-latex/unnamed-chunk-23-1.pdf}}
\end{example}
...
\pandocbounded{\includegraphics[keepaspectratio]{main_files/figure-latex/unnamed-chunk-24-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{main_files/figure-latex/unnamed-chunk-26-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{main_files/figure-latex/unnamed-chunk-29-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{main_files/figure-latex/unnamed-chunk-30-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{main_files/figure-latex/unnamed-chunk-31-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{main_files/figure-latex/unnamed-chunk-32-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{main_files/figure-latex/unnamed-chunk-33-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{main_files/figure-latex/unnamed-chunk-34-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{main_files/figure-latex/unnamed-chunk-35-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{main_files/figure-latex/unnamed-chunk-36-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{main_files/figure-latex/unnamed-chunk-36-2.pdf}}
\end{example}
...
\[\lim_{n \to \infty} \mathbb{P}(Y_n = j \mid Y_0 = j) = \pi(i).\]
...
\pandocbounded{\includegraphics[keepaspectratio]{main_files/figure-latex/unnamed-chunk-37-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{main_files/figure-latex/unnamed-chunk-37-2.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{main_files/figure-latex/unnamed-chunk-38-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{main_files/figure-latex/unnamed-chunk-39-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{main_files/figure-latex/unnamed-chunk-40-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{main_files/figure-latex/unnamed-chunk-40-2.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{main_files/figure-latex/unnamed-chunk-42-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{main_files/figure-latex/unnamed-chunk-42-2.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{main_files/figure-latex/unnamed-chunk-43-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{main_files/figure-latex/unnamed-chunk-43-2.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{main_files/figure-latex/unnamed-chunk-43-3.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{main_files/figure-latex/unnamed-chunk-43-4.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{main_files/figure-latex/unnamed-chunk-43-5.pdf}}
\end{example}
...
\pandocbounded{\includegraphics[keepaspectratio]{main_files/figure-latex/unnamed-chunk-44-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{main_files/figure-latex/unnamed-chunk-45-1.pdf}}
\end{example}
...
\pandocbounded{\includegraphics[keepaspectratio]{main_files/figure-latex/unnamed-chunk-46-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{main_files/figure-latex/unnamed-chunk-47-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{main_files/figure-latex/unnamed-chunk-48-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{main_files/figure-latex/unnamed-chunk-49-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{main_files/figure-latex/unnamed-chunk-50-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{main_files/figure-latex/unnamed-chunk-51-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{main_files/figure-latex/unnamed-chunk-52-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{postcode.jpeg}}
...
\pandocbounded{\includegraphics[keepaspectratio]{main_files/figure-latex/unnamed-chunk-54-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{main_files/figure-latex/unnamed-chunk-56-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{main_files/figure-latex/unnamed-chunk-56-2.pdf}}
\end{example}
...
\pandocbounded{\includegraphics[keepaspectratio]{main_files/figure-latex/unnamed-chunk-58-1.pdf}}
