...
1. Inference for quantities that are unobserved or hasn't happened yet. Examples of this might be the size of a payout an insurance company has to make, or a patients outcome in a clinical trial had they been received a certain treatment. 
...
::: {.definition} 
Let $X$ be a random sample from a probability distribution $\theta$. A **100(1-\alpha)% confidence interval** for $\theta$ is an interval $(u(X), v(X))$ such that 
$$
\mathbb{P}(u(X) < \theta < v(X)) = 1-\alpha
$$
:::

This means that if you had an infinite number of samples for $X$ and the corresponding infinite number of confidence intervals, then $100(1-\alpha)$% of them would contain the true value of $\theta$. It does *not* mean that there is a $100(1-\alpha)$ probability a particular interval contains the true value of $\theta$. 
...
```
Under H~0~, over repeated sampling the distribution of the $\chi^2$ statistic follows a $\chi^2$ distribution with 4 degrees of freedom. Therefore, the probability of seeing something as extreme as this (the p-value) is 
```{r}
1 - pchisq(chi.sq, 4)
```
In other words, there is no significant evidence in the data to reject H~0~ at the 10\% level.

Consider now a second scientist who is more open-minded than their colleague. They hypothesise only that $X \sim Bin(4, p)$ where $p$ is unknown. To test their H~0~ they first calculates the maximum likelihood estimator of $p$, $\hat{p}$ = 0.5, computes the values of $E[X]$ using this estimate and calculates $\chi^2$ to be 6.67 like their colleague.
