...
plot(x, y, xlab = "x", ylab = "f(x)", ylim = c(0, 4), xlim = c(-5, 10))
lines(x.star, mu.star)
polygon(c(x.star, rev(x.star)), c(f.lower, rev(f.upper)), col = rgb(0,0, 1, 0.25), border = FALSE)
#Once you go past the last data point you start sampling purely from the prior distribution
