...
mean(y)
```
Suppose that, somehow, we know the population standard deviation is 0.01$ms$. Derive a posterior distribution for $\mu$. 

Suppose the police pull over a lorry driver after they failed to stop at a red light. During a test at the side of the road, the lorry driver's reaction time is tested and comes out as 77ms. 
...
::: {.exercise}
Consider the motivation in the lecture notes for invariant prior distributions. Show that if $\theta \sim U[0, 1]$ and $\psi = \theta^2$ then $\pi(\psi) = \frac{1}{2\sqrt{\psi}}$ for $\psi \in [0, 1]$. 
:::

::: {.exercise}
Consider Example \@ref(exm:exponential) featuring the exponential distribution. 
...
<!--chapter:end:06-computation.Rmd-->
