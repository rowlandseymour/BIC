...
\begin{verbatim}
##  [1]  0.02630280  0.15897398  0.07743111 -0.55770066  0.63129461  0.42662953
##  [7]  0.09940611 -0.01219736  0.11839917  0.96228538
\end{verbatim}
...
This definiton entails that $F^{-1}$ is left-continuous (while the \textit{quantile function} \(q(u) = \inf\{x \in \mathbb{R}: F(x) > u\}\) is right-continuous). The reason for us to not demand a strict inequality for the generalised inverse is that our definition provides
\begin{equation}\label{eq:pseudo}
\forall x \in \mathbb{R}, u \in [0,1]: u \leq F(x) \iff F^{-1}(u) \leq x. 
\end{equation}
We also note that if \(F\) is bijective, its inverse function is identical to the generalised inverse.

\begin{theorem}
Let \(F :\mathbb{R} \rightarrow [0, 1]\) be a  distribution function, \(U \sim U[0, 1]\) and \(Y = F^{-1}(U)\). Then \(Y\) has distribution function \(F\).
\end{theorem}
...
%Since \(\inf\{x \in\mathbb{R} : F(x) \geq U\} \leq a\) is equivalent to \(U \leq F(a)\), we have
%\[
...
\begin{definition}
\protect\hypertarget{def:Markov}{}\label{def:Markov}A sequence of random variables \(Y = (Y_n)_{n \in \mathbb{N}_0}\) is a \textbf{Markov chain} if \(\mathbb{P}(Y_{n+1} \in \cdot \mid Y_{n}, \ldots, Y_0) = \mathbb{P}(Y_{n+1} \in \cdot \mid Y_{n})\) a.s. That is, the distribution of the next state \(Y_{n+1}\) conditional on its entire past only depends on the current state \(Y_n\).
\end{definition}

\begin{definition} 
A Markov chain is called \textbf{homogeneous} if for any $n,m \in \mathbb{N}_0$ it holds that $\mathbb{P}(Y_{n+ m} \in \cdot \mid Y_n) = \mathbb{P}(Y_{m} \in \cdot \mid Y_0)$. That is, transition probabilities only depend on the time differences.
\end{definition}

From here on, we will only deal with homogeneous Markov chains and will therefore synonymously use the terms \textit{Markov chain} and \textit{homogeneous Markov chain}.

\begin{definition} 
If for some finite set \(S = \{i_1,\ldots,i_N\}\) we have \(\mathbb{P}(Y_n \in S) = 1\) for all $n \in \mathbb{N}_0$, we call \(Y\) a \textbf{finite Markov chain} and \(S\) its \textbf{state space}.
\end{definition}

By relabelling, we may always assume wlog that \(S = \{1,\ldots,N\} \) for a finite Markov chain with $N$ states.

\begin{definition}
For a finite Markov chain the probability of transitioning from state \(i\) to state \(j\) in a Markov chain is given by \(p_{ij}\). The \textbf{transition matrix} for a Markov chain with \(N\) states is the \(N \times N\) matrix \(P = (p_{i,j})_{i,j=1,\ldots,n}\), where the \(\{i, j\}^{th}\) entry  \(p_{ij}\) is the probability  of moving from state \(i\) to state \(j\), i.e., \(\mathbb{P}(Y_{n+1} = i \mid Y_n = j) = \mathbb{P}(Y_{1} = i \mid Y_0 = j).\)
\end{definition}
...
\begin{definition}
For a finite Markov chain, the \textbf{period }of a state \(i\) is given by \(d_i = \textrm{gcd}\{n > 0; p_{ii} > 0 \}\). A state is \textbf{aperiodic} if \(d_i = 1\). An \textbf{aperiodic chain} is a chain where all states are aperiodic.
\end{definition}

\begin{definition}
A finite Markov chain is \textbf{irreducible} if for any \(i,j \in S\) there exists an \(n \in \mathbb{N}_0\) such that \(\mathbb{P} (Y_n = j \mid Y_0 = i) > 0\). In other words, it is possible to move from any state to any other state in a finite number of steps.
\end{definition}
...
\pi P = \pi. 
\]
Then  \(\pi\) is called the \textbf{stationary distribution} of the Markov chain corresponding to \(P\).
\end{definition}

The terminology arises from the fact that if the Markov chain is started in \(\pi\), the marginal distributions \(\mathbb{P}_{Y_n}\) remain unchanged (they are \textbf{invariant} under time translation), since then 
\[q^{(n)} = \pi P^n = (\pi P)P^{n-1} = \pi P^{n-1} = qP^{(n-1)} = \cdots = \pi.\]
...
\begin{enumerate} 
\item[3.] \(\pi\) is stationary for the Markov chain.
\end{enumerate} 
Note here that for finite chains, uniqueness is automatic by irreducibility.
...
\begin{theorem}[Detailed Balance for finite chains]
...
So far, we have shown that we can use \textit{finite }state Markov chain theory to simulate from a \textit{discrete} probability distribution \(\pi\) by running a Markov chain with \(\pi\) as a stationary distribution for a sufficently long time. However, for discrete distributions this is of course overkill, since we can easily sample from such distributions via the inverse transformation method. All of the above concepts translate however in a natural way to Markov chains with more complicated state spaces, and we will just briefly comment on this now: 

The generalisation of the transition matrix is a \textit{Markov kernel} \(P \colon S \times \mathcal{S} \to [0,1]\) on a general Borel state space \((S,\mathcal{B}(S))\), which satisfies the following properties: 

\begin{enumerate} 
\item \(\forall x \in S: \quad P(x, \cdot) \text{ is a probability measure}\), 
\item \(\forall A \in \mathcal{B}(S): \quad x \mapsto P(x, A) \text{ is measurable.}\)
\end{enumerate}
...
then,
\begin{equation} 
P^{n+m} = P^n \circ P^m \tag{\text{Chapman--Kolmogorov equation}}
\end{equation}
and 
\begin{equation} 
\mathbb{P}(Y_1 \in A_1, \ldots, Y_n \in A_n \mid Y_0 = x) = \int_S P(x, dy_0)  \int_{A_1, \times \cdots \times A_n} \prod_{i=1}^n P(y_{i-1}, dy_i).
\end{equation}
Analogously to the finite state space case we then call a distribution \(\pi\) on \(S\) a  \textbf{stationary distribution} if 
\[\pi P = \pi,\]
and thus iteratively 
\[\mathbb{P}(Y_n \in \cdot \mid Y_0 \sim \pi) = \pi.\]
\begin{definition} 
A \(Y\) is a Markov chain with unique stationary distribution \(\pi\) is called \textbf{ergodic} if 
...
\end{definition}
Determining whether a general Markov chain is ergodic is a much more demanding task than in the finite state space situation. For a given Markov kernel, verification is based on checking extensions of the notions of irreducibility, aperiodicity and \textbf{recurrence} for countable Markov chains, where the latter property encodes the idea that return times to any state \(i\) should be finite almost surely. The main challenge for general state spaces is that there need not be any states \(x \in S\) such that \(P(y,\{x\}) > 0\) for any \(y \in S\) (e.g.\ when the transition probabilities are absolutely continuous). Instead of going into technical details, we only cite a quantitative result that provides \textbf{exponentially fast} convergence to the invariant distribution given explicit criteria on \(P\).

\begin{theorem}[\cite{hairer11}]
Suppose that \(Y\) satisfies the following conditions: 
\begin{enumerate} 
\item \textbf{Drift condition:} There exists a \textit{penalty function} \(V\colon S \to [1,\infty)\) and constants \(K \geq 0, \gamma \in (0,1)\) such that 
\[\forall x \in S: \underbrace{\int_S V(y)\, P(x,dy)}_{= \mathbb{E}[V(Y_1) \mid Y_0 = x]}  \leq \gamma V(x) + K. \]
\item \textbf{Minorisation condition:} There exists a constant \(\alpha \in (0,1)\) and a probability measure \(\nu\) such that 
...
\begin{corollary} 
Suppose that for some probability measure \(\nu\) and \(\alpha \in (0,1)\) the following \textbf{Doeblin minorisation criterion} holds: 
...
\end{proof}
\begin{remark} 
It can be shown that \(\kappa = 1 - \alpha, D=1\), see \cite[Theorem 16.0.2]{meyntweedie}
\end{remark}
...
\begin{theorem}[Detailed Balance for dominated chains]\label{theo:reversibility}
Let \(P\) be a Markov kernel such that for any \((x,A) \in S \times \mathcal{B}(S)\) it holds that \(P(x,A) = \int_A p(x,y) \,\mu(dy)\) for some dominating measure \(\mu\). If \(P\)  satisfies detailed balance with respect to a distribution \(\pi \ll \mu\) in the sense that 
...
\pi(\theta' \mid \boldsymbol{y})\pi(\theta \mid \theta') = \min\{\pi(\theta' \mid \boldsymbol{y})q(\theta \mid \theta'),\, \pi(\theta \mid \boldsymbol{y})q(\theta' \mid \theta)\}.
\]
Hence, \(\pi(\theta \mid \boldsymbol{y}) \pi(\theta'\mid\theta) = \pi(\theta' \mid \boldsymbol{y}) \pi(\theta\mid\theta')\) and the Markov chain satisfies detailed balance with respect to the posterior disquisition. Stationarity of the posterior distribution is now an immediate consequence of Theorem \ref{theo:reversibility}.
\end{proof}

\begin{example}
\protect\hypertarget{exm:norm}{}\label{exm:norm}

A counter monitors the time until atoms decays. It collects the data \(X_1, \ldots, X_N\) and we assume \(X_i \sim \hbox{Exp}(\lambda)\). The average time \(1/\lambda\) until these atom decays is very long, so assuming \(\lambda \sim \hbox{Beta}(\alpha, \beta)\) is reasonable.
...
\item[2.2]
  ...
  \item[\(\vdots\)]
\item[2.N] Draw a value for \(\theta_N^{(i)}\) from \(\pi(\theta_N \mid \theta_1^{(i)}, \theta_2^{(i)}, \ldots, \theta_{N-1}^{(i)}))\).
